...
public class MainController {
	...
	public static void addEmployee(Person p) {
		...
		Optional<Rowable> existingPerson = dao.getEmployees().stream().filter(x -> p.getId().equals(x.getId())).findFirst();
		if (existingPerson.isPresent()) {
			// update existing record
			System.out.println("found it");
			Person e = (Person) existingPerson.get();
			p.copy(e);
			dao.updateEmployee(p);
		} else {
			dao.addEmployee(p);
		}
		_instance.drawTable(dao.getEmployees());
	}

	public static void addCustomer(Person p) {
		// check for existing record
		Optional<Rowable> existingPerson = dao.getCustomers().stream().filter(x -> p.getId().equals(x.getId())).findFirst();
		if (existingPerson.isPresent()) {
			// update existing record
			System.out.println("found it");
			Person e = (Person) existingPerson.get();
		...
		_instance.drawTable(dao.getCustomers());
	}

	public static void addProduct(Product p) {
		// check for existing record
		Optional<Rowable> existingProduct = dao.getMerchandise().stream().filter(x -> p.getId().equals(x.getId())).findFirst();
		if (existingProduct.isPresent()) {
			// update existing record
			System.out.println("found it");
			Product e = (Product) existingProduct.get();
			p.copy(e);
			dao.updateMerchandise(p);
		} else {
			dao.addMerchandise(p);
		}
		_instance.drawTable(dao.getMerchandise());
	}
	...
	protected void onTableViewClick(MouseEvent event) {
		...
		if (selected == null || selected.isEmpty()) {
			...
			if ((existing = dao.getEmployees().stream().filter(x -> selected.get(0).equals(x.getId())).findFirst()).isPresent()) {
				System.out.println("existing employee.");
			} else if ((existing = dao.getCustomers().stream().filter(x -> selected.get(0).equals(x.getId())).findFirst()).isPresent()) {
				System.out.println("existing customer.");
			} else if ((existing = dao.getMerchandise().stream().filter(x -> selected.get(0).equals(x.getId())).findFirst()).isPresent()) {
	...
	public void mockCustomers(ActionEvent event) throws IOException {
		for (int i = 0; i < 30; ++i) {
			dao.addCustomer(MockUtil.mockPerson(PersonType.CUSTOMER));
		}
		showCustomers(null);
	}
	...
	public void mockEmployees(ActionEvent event) throws IOException {
		for (int i = 0; i < 30; ++i) {
			dao.addEmployee(MockUtil.mockPerson(PersonType.EMPLOYEE));
		}
		showEmployees(null);
	}
	...
	public void mockMerchandise(ActionEvent event) throws IOException {
		for (int i = 0; i < 30; ++i) {
			...
			dao.addMerchandise(MockUtil.mockProduct());
		}
